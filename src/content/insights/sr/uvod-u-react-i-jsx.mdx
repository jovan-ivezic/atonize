...
Komponenta bi generalno trebalo da bude ili **"pametna" (stateful)** ili **"glupa" (prezentaciona)** — mešanje obe odgovornosti u istoj komponenti je tačka gde kompleksnost počinje da raste. Ovo ćemo detaljno istražiti u našem tekstu o [Upravljanje State-om i Životnim Ciklusom](/insights/react-state-management-i-zivotni-ciklus).
...
Props su read-only. Nikada ne pokušavaj da menjaš prop unutar komponente — ako trebaš da menjaš podatke, "podignuti" state na roditelja ili koristiti rešenje za upravljanje state-om. Ovo detaljno obrađujemo u [Upravljanje State-om i Životnim Ciklusom](/insights/react-state-management-i-zivotni-ciklus).
...
- **Hooks** (`useState`, `useEffect`, `useContext`) — načini da dodaš logiku i side efekte komponentama → [Napredni React Hookovi](/insights/napredni-react-hookovi)
- **Next.js App Router** — obmotava React u full-stack framework sa Server Components, rutiranjem i data fetchingom → tekst o Next.js-u uskoro
